...
class MyProblem(ea.Problem): # 继承Problem父类
    ...
    def test(self, epsilon, C): # 代入优化后的C、Gamma对测试集进行检验
        # 读取测试集数据
        X_train, X_test, Y_train, Y_test = get_data(hour_num=0, transform='sin+cos',
                                            drop_time=True, scale=True)
        ...
        ngb = NGBRegressor(Base=linear_svr_learner(epsilon=epsilon, 
                                                    C=C, 
                                                    max_iter=10000), 
                        n_estimators=500,
                        verbose_eval=100,
                        learning_rate=0.01,
                        Score=MLE).fit(self.data, self.dataTarget) # 创建分类器对象并用训练集的数据拟合分类器模型k
