...
class MyProblem(ea.Problem): # 继承Problem父类
    ...
    def test(self, n_readout, n_components): # 代入优化后的C、Gamma对测试集进行检验
        ...
        # ngb = NGBRegressor(Base=esn_linear_svr_learner(n_readout=n_readout,
        ...
        #                 verbose_eval=100,
        #                 learning_rate=0.01,
        #                 Score=MLE).fit(self.data, self.dataTarget) # 创建分类器对象并用训练集的数据拟合分类器模型

        ngb = NGBRegressor(Base=esn_ridge_learner(n_readout=n_readout,
                                                n_components=n_components,
                                                alpha=0.01), 
                        n_estimators=300,
                        verbose_eval=10,
        ...
        dataTarget_predict = ngb.predict(data_test) # 采用训练好的分类器对象对测试集数据进行预测
        test_MSE = mean_squared_error(dataTarget_predict, dataTarget_test)
        print('\nTest MSE', test_MSE)

        # test Negative Log Likelihood
        Y_dists = ngb.pred_dist(data_test) # return norm method: mean std
        test_NLL = -Y_dists.logpdf(dataTarget_test).mean()
...
def subAimFunc(args):
    ...
    #                     verbose_eval=100,
    #                     learning_rate=0.01,
    #                     Score=MLE).fit(data, dataTarget) # 创建分类器对象并用训练集的数据拟合分类器模型
    ngb = NGBRegressor(Base=esn_ridge_learner(n_readout=n_readout,
                                                n_components=n_components,
                                                alpha=0.01), 
                        n_estimators=300,
                        verbose_eval=10,
                        learning_rate=0.01,
                        Score=MLE).fit(data, dataTarget) 
    dataTarget_predict = ngb.predict(data) # 采用训练好的分类器对象对测试集数据进行预测
