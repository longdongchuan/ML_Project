...
class MyProblem(ea.Problem): # 继承Problem父类
    def __init__(self, PoolType): # PoolType是取值为'Process'或'Thread'的字符串
        ...
        Dim = 5 # 初始化Dim（决策变量维数）[n_readout, n_components, damping, weight_scaling, alpha]
        varTypes = [1, 1, 0, 0, 0] # 初始化varTypes（决策变量的类型，元素为0表示对应的变量是连续的；1表示是离散的）
        lb = [1, 1, 0, 0.5, 0] # 决策变量下界
        ub = [10000, 2000, 1, 1, 1] # 决策变量上界
    ...
    def test(self, n_readout, n_components, damping, weight_scaling, alpha): # 代入优化后的参数对测试集进行检验
        # 读取测试集数据
        X_train, X_test, Y_train, Y_test = get_data(hour_num=0, transform='sin+cos',
                                            drop_time=True, scale=True)
        ...
        esn = esn_ridge_learner(
                ...
                alpha=alpha).fit(self.data, self.dataTarget) # 创建分类器对象并用训练集的数据拟合分类器模型
...
def subAimFunc(args):
    ...
    esn = esn_ridge_learner(
            ...
            damping = damping,
            weight_scaling = weight_scaling,
            alpha=alpha) # 创建分类器对象并用训练集的数据拟合分类器模型
